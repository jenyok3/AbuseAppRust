...
## Snapshot (2026-03-06) - Chrome: running detection stabilization, safe close boundaries, batch continue fixes

### Закрито в цій сесії
- Chrome / running status UI: виправлено розсинхрон між статистикою та картками — активний стан карток тепер коректно відображається для всіх running профілів.
- Chrome / pending/suppression: прибрано локальне приглушення running у sync-потоці, яке могло приховувати реально відкриті профілі у списку карток.
- Chrome / startup behavior: зменшено стартовий сплеск фальшивих відкритих профілів (guard на ранній mtime fallback після старту застосунку).
- Chrome / debug diagnostics: тимчасово додано runtime debug (process / disk-lock / final) для локалізації проблем детекції; після діагностики віджет прибрано з UI.
- Chrome / close safety: прибрано ризикові fallback-и, які могли закривати зайві профілі; повернуто безпечну модель — краще явний fail для non-bound профілю, ніж масове закриття.
- Chrome / closable model: введено перевірку closable профілів (надійна прив'язка вікна). Для профілів без binding кнопка X блокує авто-закриття.
- Chrome / single-close UX: для невдалого закриття додано явне повідомлення.
- Chrome / batch continue: виправлено повторне відкриття першої пачки після ручного закриття — вручну закриті активні профілі зараховуються як completed.
- Chrome / batch continue performance: прибрано зайві затримки при Continue, якщо профілі вже закриті вручну.
- Chrome / batch continue slots: якщо частина попередньої пачки не закрилась, наступний запуск відкриває лише вільні слоти (threads - stillRunning).
- Chrome / recent actions text: технічні повідомлення виду OPEN=x/y замінено на людські тексти (запущено ..., залишилось ..., завершено ...).

### Поточні обмеження (важливо)
- Вручну відкриті Chrome-профілі (поза додатком) не завжди мають надійну прив'язку window -> profile на Windows.
- Через це авто-закриття таких профілів через X навмисно обмежене безпечним режимом, щоб уникнути регресу "закрилися всі профілі".

### Що перевірити на наступній сесії
- Прогнати довгу батч-сесію Chrome (наприклад, 1-100, threads 5) з міксом ручних і кнопкових закриттів та перевірити, що Continue стабільно рухається без ре-лаунчу старої пачки.
- Перевірити, що у віджеті Останні дії на Chrome більше не з'являються технічні рядки, а лише нові людські формулювання.
- За потреби: прибрати або спростити внутрішні debug-команди в Tauri backend, якщо більше не потрібні для діагностики.
## Snapshot (2026-03-06) - Theme panel + Chrome running profiles diagnostics

### Закрито в цій сесії
- Theme panel (`Тема | Ефекти`): прибрані заголовки назв у картках ефектів.
- Theme panel: після вибору ефекту меню автозакривається.
- Chrome / Windows: прибрано миготіння `powershell.exe` при опитуванні статусу профілів (прихований запуск через `CREATE_NO_WINDOW`).
- Chrome / running detection: додано нормалізацію імен профілів (`Profile N`) у фронтенді і бекенді.
- Chrome / running detection: розширено парсинг `CommandLine` (варіанти `--` і `/prefetch:*`, `%ENV%`, `\\?\` префікси, canonical path).
- Chrome / running detection: додано fallback-логіку для визначення профілю з `--user-data-dir` (basename `Profile N`), коли `--profile-directory` відсутній.
- Chrome / running detection (Windows): розширено відбір процесів (не лише strict `chrome.exe` гілка з profile args).
- Chrome / running detection: додано дисковий fallback "recently active profiles" (файли `Preferences`/`Current/Last Session/Tabs`) при активному Chrome.

### Діагностика (важливо)
- На машині користувача у `Win32_Process` стабільно видно тільки один явний профільний маркер: `--profile-directory="Profile 1"`.
- Інші відкриті вікна Chrome не експонують профіль у `CommandLine`, тому process-only детекція недостовірна.
- `settings.json` додатку: `chromeFolderPath` порожній, використовується default `C:\Users\Admin\AppData\Local\Google\Chrome\User Data`.

### Перевірки
- `cargo check` - проходив після кожного блоку Rust-змін.
- `npm run check` - проходив після кожного блоку frontend/TS-змін.

### Відкриті питання
- Попри fallback-и, користувач все ще повідомляє, що в статистиці/картках бачить тільки `Profile 1`.
- На наступній сесії перший крок: додати тимчасовий debug-вивід у Chrome UI (сирий список `get_running_chrome_profiles` + джерело `process`/`disk`) для точного підтвердження фінальної гілки детекції на runtime.
...
## Snapshot (2026-03-05) - Sidebar/чолка: повернення логотипа та вирівнювання

### Зроблено в цій сесії
- Sidebar: прибрано лівий індикатор активних іконок.
- Sidebar: прибрано лінію між логотипом та секцією іконок.
- Sidebar/Header: після експерименту з винесенням логотипа в чолку виконано відкат — логотип повернуто в сайдбар.
- Header: кнопку згорнути/розгорнути сайдбар повернуто на штатне місце в лівій капсулі.
- Sidebar: повернуто напис `AbuseApp` біля логотипа в розгорнутому стані.
- Sidebar: виконано покрокове вирівнювання логотипа (розмір, зсув вгору/вниз, горизонтальне центрування для expanded/collapsed).
- Sidebar: опущено секцію іконок трохи вниз для узгодження вертикальних відступів.
- Header: вирівняно внутрішні відступи кнопки згорнути/розгорнути (рівні візуальні падінги).
- Sidebar overlay: при розгортанні посилено затемнення та blur фону (`bg-black/35`, `backdrop-blur-md`).

### Змінені файли
- `client/src/App.tsx`
- `client/src/components/Sidebar.tsx`
- `notes/next-session.md`

### Перевірка
- `npm run check` — проходив після кожної суттєвої зміни.
