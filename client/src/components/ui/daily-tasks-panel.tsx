    ...
    <ScrollArea className="h-full">
      <div className="space-y-3">
        {tasks.map((task) => (
          <div
            key={task.id}
            className="flex items-center gap-3 p-3 rounded-xl bg-black/20 border border-white/5 hover:bg-black/30 transition-all"
          >
            <Button
              variant="ghost"
              size="sm"
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-medium truncate ${
                taskStates[task.id] ? 'line-through text-muted-foreground' : ''
              }`}>
                {task.title}
              </p>
            </div>
